...
once it was clear that The Fortuneteller's Hand — an already-designed,
original solo TTRPG — mapped onto MCP's three primitives more naturally than
anything invented for the purpose: the 52 fortune-phrases as a natural
...
## Decision 1: resources are pull-only — found by testing, not by reading the spec
...
reading the resource at all.

The cause, confirmed against MCP's actual documentation rather than assumed:
resources are designed to be pulled by a human, explicitly — a `+` menu pick
in Claude Desktop, an `@`-mention in Claude Code. There is no path for the
...
## Decision 2: a prompt can just run code
...
character was — checked against the game's own canon, this broke the
intended order (meet the Fortuneteller → reveal the cross → describe the
...
for each beat — card *drawing* stays deterministic in code; only reveal
...
## Decision 3: two independent decks, not one shuffled pool

Fate's deck (`thesitting.py`'s `SUIT_PILES`) is used entirely during the
Sitting itself — the four cross draws and the Goal deal — and never touched
...
Called Hand's landings, and the fresh debt cards defiance deals.
...
## Decision 4: a reshuffle has to reason about what's still owed
...
- **Onboarding is undesigned.** Nothing in the running server explains
  itself to a first-time player; this document and `RULES.md` are the only
  guides that exist.

None of this is an oversight so much as the honest next layer of work — the
gap between a real MCP server built around a genuinely hard state problem,
and one built the way you'd actually ship it.
